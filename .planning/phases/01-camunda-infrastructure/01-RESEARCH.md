...
**Domain:** Camunda 7 Run (standalone JAR) — PostgreSQL setup, BPMN deployment, backend proxy
**Confidence:** HIGH
...
|----|-------------|-----------------|
| INFRA-01 | Camunda 7.23.0 Run (standalone JAR) starts without Docker on macOS, backed by existing PostgreSQL (`doorman_db`) instead of H2 | PostgreSQL JDBC driver placement, `application.yml` datasource config, Java 21 confirmed available on machine |
| INFRA-02 | Backend deploys all 39 BPMN files to Camunda via REST on startup, using `deploy-changed-only` to prevent version proliferation on restart | `deployBpmn.mjs` multipart POST pattern, native FormData in Node 25, recursive directory scan |
| INFRA-03 | All frontend → Camunda REST calls are proxied through `/api/camunda/*` routes in `demo-server.mjs` (avoids CORS, enforces auth) | Proxy route block pattern, thin passthrough with no Camunda CORS config needed |
</phase_requirements>
...
Phase 1 delivers exactly three things: Camunda 7 running on PostgreSQL, all 39 BPMNs deployed (version 1 only), and a backend proxy route block so the frontend never calls Camunda directly. These are pure infrastructure and configuration tasks — no new business logic, no frontend pages, and no external task worker (that is Phase 2).

The machine environment is confirmed: Java 21 (OpenJDK, Homebrew) is available at `/usr/local/Cellar/openjdk@21/21.0.7/`; Node.js 25.6.1 is running (`demo-server.mjs`). The existing PostgreSQL at `postgresql://doorman_user:doorman_pass@localhost:5432/doorman_db` is the target database. No Docker is involved.

One critical finding from reading the actual BPMN files: all 39 processes use `camunda:delegateExpression="${delegateName}"` (Java Spring Bean delegation), NOT `camunda:type="external"` with a topic name. This means the service tasks in these BPMNs are designed for Java delegates, not for the external task worker pattern. **For Phase 1, this does not block deployment or Cockpit visibility.** Camunda will accept and deploy these BPMNs successfully regardless of implementation type. However, the planner must note this as a Phase 2 concern: the BPMNs will need to be converted from `delegateExpression` to `camunda:type="external"` with named topics before the Node.js external task worker can execute any service task. This BPMN migration is out of Phase 1 scope but must be surfaced.

**Primary recommendation:** Download Camunda 7.23.0, configure `application.yml` with PostgreSQL datasource and `deploy-changed-only=true`, drop the PostgreSQL JDBC driver JAR into `configuration/userlib/`, start the JAR, then add `deployBpmn.mjs` to `demo-server.mjs` startup and a thin `/api/camunda/*` proxy block.
...
|-----------|---------|---------|--------------|
| Camunda BPM Run | 7.23.0 | Standalone process engine JAR | Only supported non-Docker Camunda 7 distribution; REST API at `localhost:8080/engine-rest` |
| Java (OpenJDK) | 21 (already installed) | JVM for Camunda JAR | Java 11+ required; Java 21 confirmed on this machine via Homebrew |
| PostgreSQL JDBC driver | 42.7.x | Connects Camunda to PostgreSQL | Not bundled in Camunda Run; must be placed in `configuration/userlib/` |
...
| `node-fetch` / native `fetch` | built-in (Node 18+) | HTTP calls to Camunda REST | Already used in demo-server.mjs via global `fetch` |
...
# From Maven Central:
curl -O https://repo1.maven.org/maven2/org/postgresql/postgresql/42.7.4/postgresql-42.7.4.jar
```
...
```
doorman/
├── camunda-bpm-run/                    # NEW: Camunda 7 installation directory
│   ├── configuration/
│   │   ├── default.yml                 # MODIFIED: PostgreSQL datasource
│   │   └── userlib/
│   │       └── postgresql-42.7.4.jar  # NEW: JDBC driver (not bundled)
│   └── start.sh                        # Camunda startup script
│
└── backend/
...
### Pattern 1: PostgreSQL Configuration for Camunda Run

**What:** Camunda Run uses YAML for configuration. The file is `configuration/default.yml` inside the unzipped Camunda Run directory. Override the H2 datasource with PostgreSQL credentials.

**Critical:** The JDBC driver JAR must be physically present in `configuration/userlib/` before starting. Camunda Run scans this directory on boot.
...
camunda.bpm:
  database:
    schema-update: true   # auto-create Camunda tables in doorman_db on first start
  admin-user:
    id: demo
    password: demo
    firstName: Demo
...
camunda.bpm.run.cors:
  enabled: false  # Disabled: all Camunda calls go through Node.js proxy, never from browser directly
```

**Verification:** After startup, check logs for `org.postgresql.Driver` (not `H2`). Check Cockpit at `http://localhost:8080/camunda/app/cockpit/` shows zero deployments (not H2 demo data).

### Pattern 2: BPMN Deployment on Server Start (deploy-changed-only)

**What:** `deployBpmn.mjs` reads all `.bpmn` files under `processes/` recursively and POSTs each to Camunda's `deployment/create` endpoint. Uses `deploy-changed-only=true` so Camunda skips unchanged files on subsequent starts (hash comparison built into Camunda).
...
**Deployment strategy:** Deploy ALL 39 files in a SINGLE deployment call (one multipart POST with all files attached). This keeps them as one deployment unit, making version management simpler. Camunda's `deploy-changed-only` then applies at the file level within the deployment.
...
export async function deployAllBpmns() {
  ...
  console.log(`[Camunda] Deployed ${bpmnFiles.length} BPMNs. Deployment ID: ${result.id}`);
  return result;
}
```
...
client.connect(async (err) => {
  ...
  // Deploy BPMNs to Camunda on startup (idempotent)
  try {
    await deployAllBpmns();
  } catch (e) {
    console.warn('[Camunda] BPMN deployment failed (Camunda may not be running):', e.message);
    // Non-fatal: server continues; BPMNs will deploy on next start when Camunda is up
  }
});
```

### Pattern 3: Camunda Proxy Routes Block

**What:** A route block in `demo-server.mjs` catches all requests matching `/api/camunda/*`, strips the prefix, and forwards them to Camunda's `engine-rest` endpoint. The frontend never calls port 8080 directly.
...
- **Starting Camunda before placing the JDBC driver JAR:** Camunda will silently fall back to H2 in-memory mode, create schema under H2, then fail confusingly when you add the JAR later. Always place the JAR FIRST.
...
- **Using `schema-update: create-drop`:** Will drop and recreate Camunda tables on every restart, wiping process history. Use `true` (adds missing columns only).
...
**What goes wrong:** Camunda Run ships with H2 in-memory as default. Every restart loses all process instances, deployments, and Cockpit data.

**How to avoid:** Configure `application.yml` with the PostgreSQL datasource BEFORE the first start. Place the JDBC JAR in `configuration/userlib/` first. Confirm startup log shows `org.postgresql.Driver` not `H2`.
...
**What goes wrong:** Camunda Run does not bundle the PostgreSQL driver. If the JAR is missing from `configuration/userlib/`, Camunda cannot find `org.postgresql.Driver` and either falls back to H2 or fails to start.

**How to avoid:** Download `postgresql-42.7.4.jar` from Maven Central and place it in `configuration/userlib/` before starting. The filename does not matter; Camunda scans the directory for JARs.

### Pitfall 3: BPMN Version Proliferation on Restart

**What goes wrong:** Every server start calls `deployment/create`. Without `deploy-changed-only=true`, Camunda creates a new process definition version (v2, v3...) even if nothing changed. Running instances stay pinned to their original version.
...
**What goes wrong:** All 39 BPMN files use `camunda:delegateExpression="${delegateName}"` on service tasks. This is the Java Spring Bean delegation pattern. Camunda will deploy and display these BPMNs in Cockpit without error, but when a process instance reaches a service task, Camunda will try to resolve the Spring bean and fail with a `ProcessEngineException: Unknown property used in expression: ${projectInitDelegate}`.
...
**What goes wrong:** `deployAllBpmns()` throws a network error if Camunda is not started before the backend. If this is not caught, `demo-server.mjs` exits.

**How to avoid:** Wrap the `deployAllBpmns()` call in try/catch. Log a warning but do not kill the server. The backend is useful (OMS admin, form configuration) even when Camunda is offline. BPMNs will be deployed on next startup.

### Pitfall 6: `schema-update: true` Conflicts with doorman_db Ownership

**What goes wrong:** Camunda Run's `schema-update: true` runs DDL against `doorman_db` to create its `ACT_*` tables. These tables will be owned by `doorman_user` (the credentials in `application.yml`). If `doorman_user` lacks DDL privileges (only has DML), Camunda fails with `permission denied to create table`.

**How to avoid:** Before first Camunda start, grant DDL privileges to `doorman_user` on `doorman_db`, or run the Camunda schema SQL manually as `a123` (the DB owner). Camunda's SQL scripts are in the JAR at `sql/create/postgres_engine_7.sql` and `postgres_identity_7.sql`. Alternatively, set `schema-update: create` on first start as `a123`, then switch to `true`.
...
```javascript
// Source: Camunda REST docs /deployment/create + Node.js 18+ FormData
...
### Verify PostgreSQL Connection in Camunda Startup Log

```
# What you WANT to see (PostgreSQL connected):
org.camunda.bpm.engine.impl.db.sql.DbSqlSessionFactory - Using database type: postgres
...
|--------------|------------------|--------------|--------|
| Camunda 7 full distribution (WAR deploy to Tomcat) | Camunda Run (standalone JAR, no app server) | Camunda 7.13 (2020) | No Java web server needed; single JAR startup |
| `application.properties` | `application.yml` (YAML) | Camunda Run default | YAML is the standard config format for Camunda Run |
...
1. **Camunda schema creation permissions for `doorman_user`**
   - What we know: DB is owned by `a123`; `doorman_user` has SELECT/INSERT/UPDATE grants on existing tables
   - What's unclear: Whether `doorman_user` can CREATE TABLE for `ACT_*` Camunda tables
   - Recommendation: Wave 0 task — as `a123`, pre-create Camunda schema by running the Camunda SQL scripts before starting the JAR, OR grant `doorman_user` CREATE privileges on the public schema. Check `\du doorman_user` in psql.

2. **Single deployment vs. per-file deployments**
   - What we know: Camunda `deploy-changed-only` works at the resource level within a deployment
...
|--------|----------|-----------|-------------------|-------------|
| INFRA-01 | Camunda starts with PostgreSQL, not H2 | smoke | `curl -s http://localhost:8080/engine-rest/engine | grep default` | ❌ Wave 0 |
| INFRA-01 | PostgreSQL driver loaded (not H2) | log-check | `grep 'org.postgresql' < camunda-startup.log` | ❌ Wave 0 |
...
- **Per wave merge:** Full startup verification sequence: start Camunda, start backend, run all 5 curl tests above
